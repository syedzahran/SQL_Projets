...
SELECT 
    country, SUM(total_votes) AS movie_votes
FROM
    movie m
        INNER JOIN
    ratings r ON m.id = r.movie_id
...
top_gen
WHERE r.avg_rating > 8 AND g.genre IN (top_gen.genre)
GROUP BY n.name
ORDER BY movie_count DESC
)
...
FROM movie m INNER JOIN genre g on m.id=g.movie_id
WHERE genre IN(SELECT genre FROM TOP_GEN) 
)
...
SELECT production_company,count(id) AS movie_count,
RANK() OVER(ORDER BY count(id) DESC) AS movie_rank
...
SELECT dm.name_id,name,dm.movie_id,duration,r.avg_rating,total_votes,
m.date_published,
...
name AS director_name,COUNT(movie_id) AS number_of_movies,
